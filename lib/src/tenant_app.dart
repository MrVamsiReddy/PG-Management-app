        ...
        child: Center(
          child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const Text('This is the tenant app, but your account is not a tenant account. Use the owner/admin app.', textAlign: TextAlign.center),
              const SizedBox(height: 16),
...
class _TenantShellState extends State<TenantShell> {
  ...
  Widget build(BuildContext context) {
    ...
    const pages = [TenantHome(), PaymentsScreen(), MaintenanceScreen(), VisitorsScreen(), TenantProfileScreen()];
    final destinations = [
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        toolbarHeight: 68,
        title: Row(children: [
          Container(width: 36, height: 36, decoration: BoxDecoration(color: primary, borderRadius: BorderRadius.circular(11)), child: const Icon(Icons.apartment_rounded, color: Colors.white, size: 21)),
          const SizedBox(width: 10),
          Text('PG Management', style: Theme.of(context).textTheme.titleLarge?.copyWith(letterSpacing: -.7)),
        ]),
        actions: [
          IconButton(tooltip: 'Notifications', onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const NotificationsScreen())), icon: const Icon(Icons.notifications_none_rounded)),
...
class TenantHome extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(padding: const EdgeInsets.fromLTRB(20, 14, 20, 40), children: [
        Text('Hello, ${state.displayName.split(' ').first} 👋', style: Theme.of(context).textTheme.headlineMedium),
        const SizedBox(height: 20),
        if (latest != null) _rentCard(context, state, latest) else const EmptyState(icon: Icons.receipt_long_outlined, title: 'No rent scheduled yet'),
        const SizedBox(height: 24),
        Row(children: [
          _quick(context, 'Pay rent', Icons.account_balance_wallet_outlined, const PaymentsScreen()),
          const SizedBox(width: 8),
          _quick(context, 'Raise issue', Icons.build_outlined, const MaintenanceScreen()),
          const SizedBox(width: 8),
          _quick(context, 'Add visitor', Icons.badge_outlined, const VisitorsScreen()),
          const SizedBox(width: 8),
          _quick(context, 'Updates', Icons.campaign_outlined, const AnnouncementsScreen()),
        ]),
        const SizedBox(height: 24),
        Text('Recent activity', style: Theme.of(context).textTheme.titleLarge),
        const SizedBox(height: 8),
        if (recent.isEmpty)
          const Card(clipBehavior: Clip.antiAlias, child: EmptyState(icon: Icons.notifications_none_rounded, title: 'Nothing new yet'))
        else
          Card(
            clipBehavior: Clip.antiAlias,
            child: Column(children: recent.map((n) => ListTile(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text(inr(paid ? latest.amount : latest.balance), style: Theme.of(context).textTheme.headlineLarge?.copyWith(color: Colors.white)),
            const SizedBox(height: 4),
            Text(paid ? 'Paid on ${formatDay(latest.paidDate!)} · Room ${state.currentTenantRoomLabel}' : 'Due by ${formatDay(latest.dueDate)} · Room ${state.currentTenantRoomLabel}', style: const TextStyle(color: Colors.white60)),
  ...
  Widget _quick(BuildContext context, String label, IconData icon, Widget page) => Expanded(
        child: InkWell(
          ...
          child: Container(
            ...
            child: Column(children: [
              Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: primarySoft, borderRadius: BorderRadius.circular(12)), child: Icon(icon, color: primary, size: 22)),
              const SizedBox(height: 8),
              Text(label, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 11)),
            ]),
          ),
        ),
      );
...
class TenantProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.fromLTRB(20, 20, 20, 40), children: [
      ...
      CircleAvatar(radius: 43, backgroundColor: primarySoft, child: Text(state.initials, style: const TextStyle(fontSize: 25, fontWeight: FontWeight.w800, color: primary))),
      const SizedBox(height: 13),
      Center(child: Text(state.displayName, style: Theme.of(context).textTheme.titleLarge)),
      if (state.accountEmail != null) Center(child: Text(state.accountEmail!, style: const TextStyle(fontSize: 12, color: Colors.black45))),
      Center(child: Text('${state.role.label} · Room ${state.currentTenantRoomLabel} · ${state.pgNameForTenant(state.currentTenantId)}')),
      const SizedBox(height: 28),
      Card(clipBehavior: Clip.antiAlias, child: Column(children: [
        _tile(Icons.person_outline, l.t('profile.personal'), state.profilePhone ?? l.t('profile.personalSub'), () => _editPersonal(context, state, l)),
        _tile(Icons.verified_user_outlined, l.t('profile.kyc'), 'Aadhaar ${state.currentTenant?.kyc.label.toLowerCase() ?? 'pending'}', () => _kyc(context, state, l)),
        _tile(Icons.settings_outlined, l.t('profile.settings'), l.t('profile.settingsSub'), () => Navigator.push(context, MaterialPageRoute(builder: (_) => const SettingsScreen()))),
        _tile(Icons.help_outline, l.t('profile.help'), l.t('profile.helpSub'), () => _help(context, l)),
      ])),
      const SizedBox(height: 18),
      OutlinedButton.icon(onPressed: state.logout, icon: const Icon(Icons.logout), label: Text(l.t('common.signOut')), style: OutlinedButton.styleFrom(foregroundColor: const Color(0xFFC94444), padding: const EdgeInsets.all(15))),
      const SizedBox(height: 16),
  ...
  void _editPersonal(BuildContext context, AppState state, AppLocalizations l) {
    ...
    showAppSheet(context, Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      FilledButton(onPressed: () async {
        final error = await state.updatePersonalDetails(name: name.text, phone: phone.text);
  ...
  void _kyc(BuildContext context, AppState state, AppLocalizations l) {
    ...
    showAppSheet(context, Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 14),
      if (doc != null) ClipRRect(borderRadius: BorderRadius.circular(14), child: base64Image(doc, height: 180)) else const EmptyState(icon: Icons.badge_outlined, title: 'No document uploaded yet'),
  ...
  void _help(BuildContext context, AppLocalizations l) {
    showAppSheet(context, Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 16),
      ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.mail_outline, color: primary), title: Text(l.t('help.email')), subtitle: const Text('support@pgmanagement.app')),
      ListTile(contentPadding: EdgeInsets.zero, leading: const Icon(Icons.call_outlined, color: primary), title: Text(l.t('help.call')), subtitle: const Text('+91 98765 43210')),
      const SizedBox(height: 12),
