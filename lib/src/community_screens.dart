        ...
        ...state.visitors.map((visitor) => Card(
          ...
          child: Padding(padding: const EdgeInsets.all(15), child: Column(children: [
            Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(visitor.name, style: const TextStyle(fontWeight: FontWeight.w800)), Text('For ${state.tenantName(visitor.tenantId)} · ${visitor.purpose}', style: const TextStyle(fontSize: 11))])),
              StatusPill(visitor.status.label),
            ]),
            const Divider(height: 23),
            Row(children: [const Icon(Icons.schedule, size: 16, color: Colors.black45), const SizedBox(width: 5), Text(formatWhen(visitor.expectedAt), style: const TextStyle(fontSize: 11)), const Spacer(),
              if (visitor.status == VisitorStatus.awaitingApproval) ...[
                TextButton(onPressed: () => state.setVisitorStatus(visitor.id, VisitorStatus.declined), child: const Text('Decline')),
                FilledButton(onPressed: () => state.setVisitorStatus(visitor.id, VisitorStatus.inside), style: FilledButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 13, vertical: 9)), child: const Text('Approve')),
              ] else if (visitor.status == VisitorStatus.inside)
                OutlinedButton(onPressed: () => state.setVisitorStatus(visitor.id, VisitorStatus.checkedOut), style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8)), child: const Text('Check out')),
            ]),
          ])),
        )),
      ]),
  ...
  void _addVisitor(BuildContext context, AppState state) {
    ...
    showAppSheet(context, StatefulBuilder(builder: (context, setModalState) => Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const SizedBox(height: 20), FilledButton(onPressed: () {
        ...
        state.addVisitor(name: name.text.trim(), tenantId: tenantId, purpose: purpose.text.trim().isEmpty ? 'Visit' : purpose.text.trim());
      ...
      body: ListView.separated(
        ...
        itemBuilder: (context, index) {
          ...
          return Card(color: item.read ? Colors.white : primarySoft.withValues(alpha: .55), child: ListTile(
            ...
            subtitle: Text('${item.body}\n${relativeTime(item.createdAt)}', maxLines: 2),
            isThreeLine: true,
            trailing: item.read ? null : Container(width: 8, height: 8, decoration: const BoxDecoration(color: coral, shape: BoxShape.circle)),
