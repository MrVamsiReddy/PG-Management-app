...
- **Invite email (2026-07-11):** the `invite` function sends a localized (en/hi/te) invite email via **Gmail SMTP** when `GMAIL_USER`/`GMAIL_APP_PASSWORD` are set (name, email, temp password only when just generated, web login URL, APK URL, invite link, setup instructions, expiry) and returns `emailSent`; the app share-sheet is the fallback. Passwords are never logged or redisplayed. ✅ (delivery needs a Google App Password; ~500 recipients/day limit)
