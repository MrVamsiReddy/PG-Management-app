...
- ~~**P0 · Tenant can mark rent PAID.**~~ **Fixed (P9):** `payRent` removed; tenants only create `pending_confirmation` rows in `payment_submissions` (tenant has no UPDATE policy) and `007` revokes tenant write access to the `app_data` payments blob. Only owner confirmation flips a due to paid. This part is enforced by real RLS even though the dues themselves still live in the `app_data` blob.
